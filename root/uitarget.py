...
ROOT = "d:/ymir work/ui/game/target_menu/"
TARGET_HEIGHT = 50

FACE_IMAGE_DICT = {
	playerSettingModule.RACE_WARRIOR_M	: ROOT + "faces/icon_mwarrior.png",
	playerSettingModule.RACE_WARRIOR_W	: ROOT + "faces/icon_wwarrior.png",
	playerSettingModule.RACE_ASSASSIN_M	: ROOT + "faces/icon_mninja.png",
	playerSettingModule.RACE_ASSASSIN_W	: ROOT + "faces/icon_wninja.png",
	playerSettingModule.RACE_SURA_M		: ROOT + "faces/icon_msura.png",
	playerSettingModule.RACE_SURA_W		: ROOT + "faces/icon_wsura.png",
	playerSettingModule.RACE_SHAMAN_M	: ROOT + "faces/icon_mshaman.png",
	playerSettingModule.RACE_SHAMAN_W	: ROOT + "faces/icon_wshaman.png",
}


if app.ENABLE_WOLFMAN_CHARACTER:
	FACE_IMAGE_DICT.update({playerSettingModule.RACE_WOLFMAN_M  : ROOT + "faces/icon_mlykaner.png",})
...
class TargetBoard(ui.ThinBoard):
		...
		class InfoBoard(ui.ThinBoard):
			class ItemListBoxItem(ui.ListBoxExNew.Item):
				def __init__(self, width):
					...
					chanceLine = ui.TextLine()
					chanceLine.SetParent(nameLine)
					chanceLine.SetPosition(32 + 65 + 33, -1)
					chanceLine.Show()
					self.chanceLine = chanceLine
				...
				def SetChance(self, text):
					self.chanceLine.SetText(text)
			...
			def __UpdatePosition(self, targetBoard):
				self.SetPosition(targetBoard.GetLeft() + (targetBoard.GetWidth() - self.GetWidth()) / 2 + 13, targetBoard.GetBottom() - 17)
					...
					if len(constInfo.MONSTER_INFO_DATA[race]["items"]) == 0:
						...
						for curItem in constInfo.MONSTER_INFO_DATA[race]["items"]:
							if curItem.has_key("vnum_list"):
								height += self.AppendItem(itemListBox, curItem["vnum_list"], curItem["count"], curItem["chance"])
							else:
								height += self.AppendItem(itemListBox, curItem["vnum"], curItem["count"], curItem["chance"])
						...
						if len(constInfo.MONSTER_INFO_DATA[race]["items"]) > itemListBox.GetViewItemCount():
							...
							itemScrollBar.SetPosition(itemListBox.GetRight()+15, itemListBox.GetTop())
							...
							# itemScrollBar.SetScrollEvent(ui.__mem_func__(self.OnScroll))
							itemScrollBar.Show()
			...
			def AppendSeperator(self):
				img = ui.ImageBox()
				img.LoadImage("d:/ymir work/ui/game/seperator.tga")
			...
			def AppendItem(self, listBox, vnums, count, chance):
				if type(vnums) == int:
					vnum = vnums
				else:
					vnum = vnums[0]
				...
				myItem.SetChance(self.GetChanceCategory(chance))
			...
			def GetChanceCategory(self, chance):	
				# chat.AppendChat(1, "Chance is %d" % chance)
				...
				if chance >= 800:
					return "|cffffffffGarantat"
				# elif chance <= 100 and chance > 89:
					# return "|cFFdbad49Comun"
				elif chance <= 790 and chance > 149:
					return "|cFFdbad49Comun"
				elif chance <= 148 and chance > 100:
					return "|cff00ffffRar"
				elif chance <= 100 and chance > 50:
					return "|cFFFFA500Foarte Rar"
				elif chance <= 50 and chance > 2:
					return "|cFF9370DBMitic"
				elif chance <= 2 and chance >= 0:
					return "|cffffcc00Legendar"
	...
	def __init__(self):
		...
		self.SetSize(400, TARGET_HEIGHT)
		
		self.rotation = 0.100

		name = ui.TextLine()
		name.SetParent(self)
		name.SetWindowHorizontalAlignCenter()
		name.SetHorizontalAlignCenter()
		name.SetDefaultFontName()
		name.SetOutline()
		name.Show()

		hpPercentage = ui.TextLine()
		hpPercentage.SetParent(self)
		hpPercentage.SetWindowHorizontalAlignLeft()	
		hpPercentage.SetHorizontalAlignLeft()				
		hpPercentage.SetPosition(160, 17)
		hpPercentage.Hide()

		if app.ENABLE_ELEMENT_ADD:
			elementImage = ui.ImageBox()
			elementImage.SetParent(self)
			elementImage.SetPosition(45+15 - 50 - 35 - 60, 9)
			elementImage.SetWindowHorizontalAlignLeft()
			elementImage.Hide()

		if app.ENABLE_ELEMENT_TARGET:
			elementIMG = ui.ImageBox()
			elementIMG.SetParent(self)
			elementIMG.SetPosition(45+15 - 50 - 35 - 60, 9)
			elementIMG.SetWindowHorizontalAlignLeft()
			elementIMG.Hide()
			
		hpText = ui.TextLine()
		hpText.SetParent(self)
		hpText.SetDefaultFontName()
		hpText.SetWindowHorizontalAlignLeft()	
		hpText.SetHorizontalAlignLeft()		
		hpText.SetOutline()
		hpText.Hide()


		hpNpSpacer = ui.ImageBox() 
		hpNpSpacer.SetParent(self)
		hpNpSpacer.AddFlag("not_pick")
		hpNpSpacer.SetPosition(45+15 - 50 - 35 - 25, 15 + 30 + 20)
		hpNpSpacer.LoadImage("d:/ymir work/ui/game/comp/spacer.png")
		hpNpSpacer.SetWindowHorizontalAlignLeft()
		hpNpSpacer.Hide()

		hpNewText = ui.TextLine()
		hpNewText.SetParent(hpNpSpacer)
		hpNewText.SetPosition(43,7)
		hpNewText.SetVerticalAlignCenter()
		hpNewText.SetHorizontalAlignCenter()
		hpNewText.SetDefaultFontName()
		hpNewText.SetOutline()
		hpNewText.SetText("")
		hpNewText.Hide()
		
		hpNewGauge = ui.ExpandedImageBox()
		hpNewGauge.SetParent(self)
		hpNewGauge.LoadImage(ROOT + "gauge_background.png")
		hpNewGauge.SetWindowHorizontalAlignLeft()
		hpNewGauge.SetPosition(-45, -10)
		hpNewGauge.AddFlag("not_pick")
		hpNewGauge.Hide()

		self.hpNewGaugePercent = ui.ImageBox() 
		self.hpNewGaugePercent.SetParent(hpNewGauge)
		self.hpNewGaugePercent.AddFlag("not_pick")
		self.hpNewGaugePercent.SetPosition(6, 5)
		self.hpNewGaugePercent.LoadImage(ROOT + "gauge_full.png")
		self.hpNewGaugePercent.Show()


		self.hpNewGaugeBackPercent = ui.ImageBox() 
		self.hpNewGaugeBackPercent.SetParent(hpNewGauge)
		self.hpNewGaugeBackPercent.AddFlag("not_pick")
		self.hpNewGaugeBackPercent.SetPosition(6, 5)
		self.hpNewGaugeBackPercent.LoadImage(ROOT + "gauge_full.png")
		self.hpNewGaugeBackPercent.Show()
		
		# After Gauge Percent to be properly show correct.
		self.faceImage = ui.ImageBox()
		self.faceImage.SetParent(hpNewGauge)
		self.faceImage.SetPosition(10, 10)
		self.faceImage.Hide()
		...
		if app.ENABLE_SEND_TARGET_INFO:
			...
			infoButton.SetUpVisual("d:/ymir work/ui/pattern_renewal/mob_info_01.tga")
			infoButton.SetOverVisual("d:/ymir work/ui/pattern_renewal/mob_info_02.tga")
			infoButton.SetDownVisual("d:/ymir work/ui/pattern_renewal/mob_info_03.tga")
			infoButton.SetPosition(50, 12)
		...
		for buttonName in self.BUTTON_NAME_LIST:
			...
			button.SetUpVisual("d:/ymir work/ui/public/target_button_01.tga")
			button.SetOverVisual("d:/ymir work/ui/public/target_button_02.tga")
			button.SetDownVisual("d:/ymir work/ui/public/target_button_03.tga")
		...
		self.buttonDict["Open"].SetEvent(ui.__mem_func__(self.OnOpenShop))
		...
		self.hpNewGauge = hpNewGauge
		self.hpNewText = hpNewText
		self.hpNpSpacer = hpNpSpacer
		if app.ENABLE_ELEMENT_ADD:
	...
	def OnOpenShop(self):
		if self.vid:
			...
			if chr.GetRace() >= 30000 and chr.GetRace() <= 30008:
	...
	def Destroy(self):
		...
		self.name = None
		self.hpPercentage = None

		self.hpNewGauge = None
		self.hpNewText = None
		self.hpNpSpacer = None
	...
	if app.ENABLE_SEND_TARGET_INFO:
		...
		def OnPressedInfoButton(self):
			...
			self.infoButton.showWnd.BindInterface(self.interface)
	...
	def Close(self):
		...
		if app.ENABLE_SEND_TARGET_INFO:
			self.infoButton.showWnd.Close()
		if app.ENABLE_ELEMENT_TARGET:
			self.elementIMG.Hide()
		self.Hide()

	def OpenForShop(self, vid, name):
		if vid:
			if vid != self.GetTargetVID():
				self.ResetTargetBoard()
				self.SetTargetVID(vid)
				self.SetTargetName(name[0:name.find("'s")])
				player.SetTarget(vid)

			self.HideAllButton()
			self.__ShowButton("Open")
			self.__ShowButton(localeInfo.TARGET_BUTTON_WHISPER)
			self.__ArrangeButtonPosition()
			# self.name.SetPosition(23 + 25, 13)		
			# self.name.SetWindowHorizontalAlignLeft()
			# self.name.SetHorizontalAlignLeft()
			self.UpdatePosition()
			self.Show()

	def Open(self, vid, name, hpPercentage, HP = -1, MaxHP = -1, isPoison = -1):
		if chr.IsSupport(vid):
			self.Hide()
			return
			
		if not self.CanShowTarget():
			return
			
		hiddenNames = ["- Support", "- Buffi", "- Healer"]
		...
		if vid:
			...
			if app.ENABLE_ELEMENT_TARGET:
				if self.elementId in ELEMENT_IMAGE_DIC.keys():
					self.elementIMG.Show()
				else:
					self.elementIMG.Hide()
			...
			if vid != self.GetTargetVID():
				...
				if nonplayer.GetRaceNumByVID(self.vid) == 9321 or self.IsPrivateShop():
	...
	def UpdatePosition(self):
		self.SetPosition(wndMgr.GetScreenWidth()/2 - self.GetWidth()/2, 24)
	...
	def GetTargetVID(self):
		return self.vid
		constInfo.TARGET_VID = self.vid
		return constInfo.TARGET_VID
	...
	def IsMyTarget(self):
		if (player.GetMainCharacterIndex() == self.vid):
			return True
		
		return False
		
	def CanShowTarget(self):
		# if (player.GetMainCharacterIndex() == self.vid):
			# return False
	
		# Another condition if in future i do another changes.
		if (player.GetMainCharacterIndex() == self.vid and player.IsMountingHorse()): 
			return False
	...
	def IsPrivateShop(self):
		# if chr.INSTANCE_TYPE_SHOP == chr.GetInstanceType(self.vid):
			# return True
		# if nonplayer.GetRaceNumByVID(self.vid) == 30000:
		if nonplayer.GetRaceNumByVID(self.vid) >= 30000 and nonplayer.GetRaceNumByVID(self.vid) <= 30008:
			return True
			
		return False

	def SetHP(self, hpPercentage, hp, maxhp):
		#if not self.CanShowTarget():
		#	return

		hptttt = ""
		if hpPercentage > 0:
			if hp > 10000 : hp = "{:,}".format(hp/1000).replace(',','.') + "k"		
			if maxhp > 10000 : maxhp = "{:,}".format(maxhp/1000).replace(',','.') + "k"
			hptttt = str("%s / %s" % (str(hp), str(maxhp)))

		self.name.SetHorizontalAlignCenter()
		self.name.SetPosition(0, 15 - 8)
		...
		if player.IsPVPInstance(self.vid):
			self.__ShowButton(18)
			self.__ArrangeButtonPosition()					

		if chr.INSTANCE_TYPE_PLAYER != chr.GetInstanceType(self.vid) and not self.IsPrivateShop():	
			self.HideAllButton()
			self.SetSize(300, TARGET_HEIGHT)
			self.name.SetPosition(0, 18)
		...
		newGaugePercent = 1.0 - float(hpPercentage / 100.0)
		self.hpNewGaugePercent.SetCoolTime(newGaugePercent)	

		oldPercentage2 = 1.0 - float(self.newPercentage / 100.0)
		self.hpNewGaugeBackPercent.SetCoolTime(oldPercentage2)
		
		self.oldPercentage = float(self.newPercentage)
		self.newPercentage = float(hpPercentage)	
		self.hpNewGaugeBackPercent.Hide()
		
		if self.faceImage.IsIn():
			self.hpNewText.Show()
		else:
			self.hpNewText.Hide()

		self.hpNpSpacer.Show()
		self.hpNewText.Show()

		self.hpNewText.SetText(hptttt)

	def showInformations(self, arg):
		if arg:
			self.hpPercentage.Show()
			
			self.hpNewGauge.Hide()
			self.hpNewText.Hide()
			self.hpNpSpacer.Hide()
			self.faceImage.Hide()
		else:
			self.hpPercentage.Hide()
			
			self.hpNewGauge.Show()
			self.hpNpSpacer.Show()
			self.hpNewText.Show()
	...
	def SetFaceImage(self):
		race = player.GetRaceNumByVid(self.vid)
		if race > 10:
			if nonplayer.IsMonsterStone(race):
				self.faceImage.LoadImage("d:/ymir work/ui/game/target_menu/faces/icon_stone.png")
			else:
				self.faceImage.LoadImage("d:/ymir work/ui/game/target_menu/faces/icon_monster.png")
		else:
	...
	def OnWhisper(self):
		if self.eventWhisper:
			self.eventWhisper(self.GetTargetName())
	...
	def __ArrangeButtonPosition(self):
		...
		pos = -(showingButtonCount / 2) * 78
		if 0 == showingButtonCount % 2:
			pos += 39

		for button in self.showingButtonList:
			button.SetPosition(pos, 22)
			pos += 78

		if showingButtonCount <= 6:
			self.SetSize(max(150, 6* 85 + 20), TARGET_HEIGHT)
		else:
			self.SetSize(max(150, showingButtonCount * 85 + 20), TARGET_HEIGHT)

		if player.IsPVPInstance(self.vid):
			self.SetSize(200, TARGET_HEIGHT)
			
		if self.IsPrivateShop():
			self.SetSize(200, TARGET_HEIGHT+5)
			self.hpNewGauge.Hide()
			self.hpNpSpacer.Hide()
			self.hpNewText.Hide()
			self.name.SetHorizontalAlignCenter()
	...
	def OnUpdate(self):
		if self.isShowButton:
			if self.buttonDict[localeInfo.TARGET_BUTTON_EXCHANGE].IsShow():
				if player.GetCharacterDistance(self.vid) > self.EXCHANGE_LIMIT_RANGE:
					self.RefreshButton()
			else:
				if player.GetCharacterDistance(self.vid) < self.EXCHANGE_LIMIT_RANGE:
					self.RefreshButton()

		if self.hpNewGaugePercent and self.hpNewGaugePercent.IsShow() and self.oldPercentage > self.newPercentage:
			self.hpNewGaugeBackPercent.Show()

			self.oldPercentage = self.oldPercentage - 0.5
			oldPercentage2 = 1.0 - float(self.oldPercentage / 100.0)
			self.hpNewGaugeBackPercent.SetCoolTime(oldPercentage2)
		...
		def SetElementImage(self, elementId):
			try:
				if elementId > 0 and elementId in ELEMENT_IMAGE_DIC.keys():
					...
					self.elementIMG.LoadImage("d:/ymir work/ui/game/12zi/element/%s.sub" % (ELEMENT_IMAGE_DIC[elementId]))
					self.elementIMG.Show()
			except:
				pass
