...
def SaveMouseButtonSettings():
	global MOUSE_SETTINGS
	cfg.Set(cfg.SAVE_GENERAL, "MOUSE_LEFT", MOUSE_SETTINGS[0])
	cfg.Set(cfg.SAVE_GENERAL, "MOUSE_RIGHT", MOUSE_SETTINGS[1])

def LoadMouseButtonSettings():
	global MOUSE_SETTINGS
	MOUSE_SETTINGS[0] = int(cfg.Get(cfg.SAVE_GENERAL, "MOUSE_LEFT", "0"))
	MOUSE_SETTINGS[1] = int(cfg.Get(cfg.SAVE_GENERAL, "MOUSE_RIGHT", "0"))
	...
	class ExpandedMoneyTaskBar(ui.ScriptWindow):
		...
		def LoadWindow(self):
			try:
				...
				# self.wndGayaGreen = self.GetChild("gaya_green")
				self.wndGayaBlue = self.GetChild("gaya_blue")
			
				self.wndMoneyIcon = self.GetChild("Money_Icon")
				# self.wndGayaGreenIcon = self.GetChild("Gaya_Metin_Icon")
				self.wndGayaBlueIcon = self.GetChild("Gaya_Leaders_Icon")
			except:
				import exception
		...
		def RefreshStatus(self):
			# self.wndMoney.SetText(localeInfo.AddPointToNumberString(long(player.GetElk())))
			if self.wndGayaBlue:
				self.wndGayaBlue.SetText(localeInfo.AddPointToNumberString(long(player.GetGayaBlue())))
			# if self.wndGayaGreen:
				# self.wndGayaGreen.SetText(localeInfo.AddPointToNumberString(long(player.GetGayaGreen())))
...
class TaskBar(ui.ScriptWindow):
	...
	class SkillButton(ui.SlotWindow):
		...
		def __init__(self):
			...
			slotIndex = 0
			wndMgr.SetSlotBaseImage(self.hWnd, "d:/ymir work/ui/game/comp/slot.png", 1.0, 1.0, 1.0, 1.0)
	...
	def LoadWindow(self):
		try:
			pyScrLoader = ui.PythonScriptLoader()
			pyScrLoader.LoadScriptFile(self, "uiscript/TaskBar_Norm.py")
			pyScrLoader.LoadScriptFile(self.mouseModeButtonList[self.MOUSE_BUTTON_LEFT], "UIScript/MouseButtonWindow_Norm.py")
			pyScrLoader.LoadScriptFile(self.mouseModeButtonList[self.MOUSE_BUTTON_RIGHT], "UIScript/RightMouseButtonWindow_Norm.py")
		except:
			import exception
		...
		# self.GetChild("OfflineShop").Hide()
		expGauge = []
		...
		self.ManageExpBtn = self.GetChild("ManageExpButton")
		self.ManageExpBtn.SetEvent(ui.__mem_func__(self.ToggleExpButton))
		...
		self.RefreshExpButton()
	
	def ToggleExpButton(self):
		if self.timeFloodExp > app.GetGlobalTimeStamp():
			chat.AppendChat(1, localeInfo.WAIT_A_SEC)
			return

		net.SendChatPacket("/anti_exp")
		self.timeFloodExp = app.GetGlobalTimeStamp() + 2

		if constInfo.ANTI_EXP_STATUS == 1:
	...
	def RefreshExpButton(self):
		if self.ManageExpBtn == None:
			return
	
		if constInfo.ANTI_EXP_STATUS == 1:
			self.ManageExpBtn.SetUpVisual("d:/ymir work/ui/game/TaskBar/antiexp_on_01.tga")
			self.ManageExpBtn.SetOverVisual("d:/ymir work/ui/game/TaskBar/antiexp_on_02.tga")
			self.ManageExpBtn.SetDownVisual("d:/ymir work/ui/game/TaskBar/antiexp_on_03.tga")
		else:
			self.ManageExpBtn.SetUpVisual("d:/ymir work/ui/game/TaskBar/antiexp_off_01.tga")
			self.ManageExpBtn.SetOverVisual("d:/ymir work/ui/game/TaskBar/antiexp_off_02.tga")
			self.ManageExpBtn.SetDownVisual("d:/ymir work/ui/game/TaskBar/antiexp_off_03.tga")
	...
	def __RampageGauge_Click(self):
		app.ExecuteShell("https://play.deimos2.net/shop/")
		# net.SendChatPacket("/in_game_mall")

	def __LoadMouseSettings(self):
		canInitDefault = False
		...
		try:
			...
			canInitDefault = True
			import dbg
			dbg.TraceError("__LoadMouseSettings: Couldn't load custom settings from user.cfg")
		...
		if canInitDefault:
			import dbg
			dbg.TraceError("__LoadMouseSettings: Trying default settings")
		...
		for slot in self.quickslot:
			...
			for i in xrange(4):
				...
				if player.SLOT_TYPE_NONE == Type:
					...
					slot.DeactivateSlot(slotNumber) ## FIX_SLOT_HIGHLIGHT (@Grimm)
					continue
				...
				if player.SLOT_TYPE_INVENTORY == Type:
					...
					bIsActive = False # FIX_SLOT_HIGHLIGHT (@ Grimm)
					...
					if constInfo.IS_AUTO_POTION(itemIndex):
						...
						if 0 != int(metinSocket[0]):
							...
							bIsActive = True # FIX_SLOT_HIGHLIGHT (@ Grimm)
					
					if not bIsActive: # FIX_SLOT_HIGHLIGHT (@ Grimm)
